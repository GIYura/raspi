...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jura");
MODULE_DESCRIPTION("A Button/LED test driver for the raspi");
MODULE_VERSION("0.1");

static unsigned int gpioLED = 21;       /* hard coding the LED gpio for this example to GPIO21 */
static unsigned int gpioButton = 17;    /* hard coding the button gpio for this example to GPIO17) */
...
static int __init gpio_init(void)
{
    ...
    ledOn = true;
    gpio_request(gpioLED, "sysfs");             /* gpioLED is hardcoded to 21, request it */
    gpio_direction_output(gpioLED, ledOn);      /* Set the gpio to be in output mode and on */
    gpio_export(gpioLED, false);                /* Causes gpio21 to appear in /sys/class/gpio
                                                the bool argument prevents the direction from being changed */
    gpio_request(gpioButton, "sysfs");          /* Set up the gpioButton */
    gpio_direction_input(gpioButton);           /* Set the button GPIO to be an input */
    gpio_set_debounce(gpioButton, 200);         /* Debounce the button with a delay of 200ms */
    gpio_export(gpioButton, false);             /* Causes gpio17 to appear in /sys/class/gpio
                                                the bool argument prevents the direction from being changed */
    ...
    result = request_irq(irqNumber,                     /* The interrupt number requested */
                        ...
                        "gpio_handler",                 /*  Used in /proc/interrupts to identify the owner */
